#!/usr/bin/env python3
"""Deterministic repository preflight for Vexora."""
...
CONTRACT = ROOT / "contracts" / "vexora.py"

REQUIRED_FILES = [
    ROOT / "README.md",
    ROOT / "SUBMISSION.md",
    ROOT / "contracts" / "vexora.py",
    ROOT / "docs" / "CONSENSUS.md",
    ROOT / "docs" / "SECURITY.md",
    ROOT / "examples" / "consumer.py",
    ROOT / "tests" / "direct" / "test_vexora.py",
    ROOT / "tests" / "direct" / "test_vexora_hardening.py",
]
...
def main() -> int:
    ...
    checks += 1
    if "Vexora" not in class_names:
        fail("Vexora contract class missing")
    ...
    print(f"PASS: Vexora preflight ({checks} checks)")
    return 0
