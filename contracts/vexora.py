...
MAX_VEXORA_LIFETIME = 365 * 24 * 60 * 60
...
class IVexora:
    class View:
        ...
        def get_vexora(self, vexora_id: u256) -> dict: ...
        def get_vexora_lineage(self, vexora_id: u256) -> dict: ...
        def get_vexora_terms(self, vexora_id: u256) -> list: ...
        ...
        def is_vexora_active(self, vexora_id: u256, expected_agreement_hash: str) -> bool: ...
    ...
    class Write:
        ...
        def ratify_vexora(self, vexora_id: u256) -> None: ...
...
class VexoraProposed(gl.Event):
    def __init__(self, vexora_id: u256, assessment_id: u256, /, **blob): ...


class VexoraRatified(gl.Event):
    def __init__(self, vexora_id: u256, party: Address, /, **blob): ...


class VexoraActivated(gl.Event):
    def __init__(self, vexora_id: u256, agreement_hash: str, /, **blob): ...


class VexoraRejected(gl.Event):
    def __init__(self, vexora_id: u256, party: Address, /, **blob): ...


class VexoraExpired(gl.Event):
    def __init__(self, vexora_id: u256, /, **blob): ...


class VexoraSuperseded(gl.Event):
    def __init__(self, vexora_id: u256, successor_id: u256, /, **blob): ...
...
def vexora_name(value: int) -> str:
    return {
        VEXORA_PROPOSED: "PROPOSED",
        VEXORA_ACTIVE: "ACTIVE",
        VEXORA_REJECTED: "REJECTED",
        VEXORA_EXPIRED: "EXPIRED",
        VEXORA_SUPERSEDED: "SUPERSEDED",
    }.get(int(value), "UNKNOWN")
    ...
    payload = json.dumps(
        {
            ...
            "parent_vexora_id": int(parent_vexora_id),
        },
        sort_keys=True,
        separators=(",", ":"),
        ensure_ascii=True,
...
class Vexora(gl.Contract):
    ...
    def _require_vexora(self, vexora_id: u256) -> VexoraRecord:
        value = self.treaties.get(vexora_id)
        if value is None:
            raise gl.vm.UserError(f"{ERR_EXPECTED}: unknown vexora {vexora_id}")
        return value
    ...
    def open_assessment(
        ...
        if policy_a.owner == policy_b.owner:
            raise gl.vm.UserError(f"{ERR_EXPECTED}: vexora parties must be independent owners")
    ...
    def propose_vexora(
        ...
        if sender != policy_a.owner and sender != policy_b.owner:
            raise gl.vm.UserError(f"{ERR_EXPECTED}: only a vexora party may propose")
        ...
        if expiry != 0:
            ...
            if expiry - now > MAX_VEXORA_LIFETIME:
                raise gl.vm.UserError(
                    f"{ERR_EXPECTED}: vexora lifetime exceeds {MAX_VEXORA_LIFETIME} seconds"
                )

        parent_id = int(parent_vexora_id)
        if parent_id != 0:
            parent = self._require_vexora(parent_vexora_id)
            if int(parent.status) != VEXORA_ACTIVE:
                raise gl.vm.UserError(f"{ERR_EXPECTED}: parent vexora must be active")
            if not (
                ...
                raise gl.vm.UserError(f"{ERR_EXPECTED}: parent vexora parties do not match")
            if int(parent.expires_at) != 0 and now >= int(parent.expires_at):
                raise gl.vm.UserError(f"{ERR_EXPECTED}: parent vexora has expired")
        ...
        vexora.party_a = policy_a.owner
        vexora.party_b = policy_b.owner
        vexora.agreement_hash = agreement_hash
        vexora.status = u8(VEXORA_PROPOSED)
        vexora.ratified_a = sender == policy_a.owner
        vexora.ratified_b = sender == policy_b.owner
        vexora.proposed_at = u256(now)
        vexora.activated_at = u256(0)
        vexora.expires_at = u256(expiry)
        vexora.parent_vexora_id = parent_vexora_id
        ...
        VexoraRatified(vexora_id, sender).emit()
        return vexora_id

    def _activate_if_ready(self, vexora_id: u256, vexora: VexoraRecord) -> None:
        if not vexora.ratified_a or not vexora.ratified_b:
            return
        now = message_timestamp()
        if int(vexora.expires_at) != 0 and now >= int(vexora.expires_at):
            vexora.status = u8(VEXORA_EXPIRED)
            VexoraExpired(vexora_id).emit()
            return

        parent_id = int(vexora.parent_vexora_id)
        if parent_id != 0:
            parent = self._require_vexora(vexora.parent_vexora_id)
            if int(parent.status) != VEXORA_ACTIVE:
                raise gl.vm.UserError(
                    f"{ERR_EXPECTED}: parent vexora is no longer active"
                )
            parent.status = u8(VEXORA_SUPERSEDED)
            self.successors[vexora.parent_vexora_id] = vexora_id
            VexoraSuperseded(vexora.parent_vexora_id, vexora_id).emit()

        vexora.status = u8(VEXORA_ACTIVE)
        vexora.activated_at = u256(now)
        VexoraActivated(
            vexora_id,
            str(vexora.agreement_hash),
            activated_at=now,
        ).emit()

    @gl.public.write
    def ratify_vexora(self, vexora_id: u256) -> None:
        vexora = self._require_vexora(vexora_id)
        if int(vexora.status) != VEXORA_PROPOSED:
            raise gl.vm.UserError(f"{ERR_EXPECTED}: vexora is not awaiting ratification")

        assessment = self._require_assessment(vexora.assessment_id)
        policy_a = self._require_policy(assessment.policy_a_id)
        policy_b = self._require_policy(assessment.policy_b_id)
        if int(policy_a.status) != POLICY_ACTIVE or int(policy_b.status) != POLICY_ACTIVE:
            raise gl.vm.UserError(
        ...
        sender = gl.message.sender_address
        if sender == vexora.party_a:
            if vexora.ratified_a:
                raise gl.vm.UserError(f"{ERR_EXPECTED}: party A already ratified")
            vexora.ratified_a = True
        elif sender == vexora.party_b:
            if vexora.ratified_b:
                raise gl.vm.UserError(f"{ERR_EXPECTED}: party B already ratified")
            vexora.ratified_b = True
        else:
            raise gl.vm.UserError(f"{ERR_EXPECTED}: sender is not a vexora party")

        VexoraRatified(vexora_id, sender).emit()
        self._activate_if_ready(vexora_id, vexora)

    @gl.public.write
    def reject_vexora(self, vexora_id: u256) -> None:
        vexora = self._require_vexora(vexora_id)
        if int(vexora.status) != VEXORA_PROPOSED:
            raise gl.vm.UserError(f"{ERR_EXPECTED}: vexora is not awaiting ratification")
        sender = gl.message.sender_address
        if sender != vexora.party_a and sender != vexora.party_b:
            raise gl.vm.UserError(f"{ERR_EXPECTED}: sender is not a vexora party")
        vexora.status = u8(VEXORA_REJECTED)
        VexoraRejected(vexora_id, sender).emit()

    @gl.public.write
    def refresh_expiry(self, vexora_id: u256) -> None:
        vexora = self._require_vexora(vexora_id)
        if int(vexora.status) not in (VEXORA_PROPOSED, VEXORA_ACTIVE):
            return
        expiry = int(vexora.expires_at)
    ...
    @gl.public.view
    def get_vexora(self, vexora_id: u256) -> dict:
        value = self._require_vexora(vexora_id)
        effective_active = int(value.status) == VEXORA_ACTIVE
        ...
        return {
            ...
            "status": int(value.status),
            "status_name": vexora_name(int(value.status)),
            ...
            "parent_vexora_id": int(value.parent_vexora_id),
    ...
    def get_vexora_lineage(self, vexora_id: u256) -> dict:
        value = self._require_vexora(vexora_id)
        successor = self.successors.get(vexora_id)
        return {
            "vexora_id": int(vexora_id),
            "parent_vexora_id": int(value.parent_vexora_id),
            "successor_vexora_id": 0 if successor is None else int(successor),
        }
    ...
    def is_vexora_active(self, vexora_id: u256, expected_agreement_hash: str) -> bool:
        ...
        if str(value.agreement_hash) != str(expected_agreement_hash):
            return False
        if int(value.status) != VEXORA_ACTIVE:
            return False
    ...
    def status_dictionary(self) -> dict:
        return {
            ...
            "vexora": {
                "PROPOSED": VEXORA_PROPOSED,
                "ACTIVE": VEXORA_ACTIVE,
                "REJECTED": VEXORA_REJECTED,
                "EXPIRED": VEXORA_EXPIRED,
                "SUPERSEDED": VEXORA_SUPERSEDED,
            },
        }
