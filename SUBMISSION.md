...
Vexora is intentionally contract-only. It has no frontend and no hosted backend.

## One-line purpose

Vexora provides reusable consensus-backed semantic compatibility receipts for two immutable natural-language policy versions, then requires bilateral owner ratification before an agreement becomes active.

## Why this is a primitive

The contract stops before any product-specific action. It does not transfer funds, execute an agent task, route an API call, settle a purchase, or provide a user interface. Downstream contracts consume its assessment and vexora state.

## What is novel in the state design

Vexora deliberately separates two different trust problems.
...
A `VexoraRecord` references a compatible assessment but does not become active until both independent policy owners ratify.
...
Vexora stores the original clauses from both pinned policy versions. Consensus only determines whether overlapping clauses can coexist.
...
`tests/direct/test_vexora.py`

`tests/direct/test_vexora_hardening.py`
...
`IVexora` is declared in the contract.

A consumer can pin both vexora ID and agreement hash:

```python
vexora.is_vexora_active(vexora_id, expected_agreement_hash)
```
...
7. Bob ratifies and the vexora becomes `ACTIVE`.
8. Create a successor proposal referencing the active vexora.
