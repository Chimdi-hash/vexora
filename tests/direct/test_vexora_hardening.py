...
def test_pause_blocks_new_proposal_and_pending_consent(direct_vm, direct_deploy, direct_alice, direct_bob):
    ...
    with direct_vm.expect_revert("active to propose"):
        contract.propose_vexora(assessment_id, 0, 0)


def test_active_vexora_remains_active_when_policy_paused(direct_vm, direct_deploy, direct_alice, direct_bob):
    contract, _, a, b = create_pair(direct_vm, direct_deploy, direct_alice, direct_bob, B.replace("email", "pii"))
    assessment_id = contract.open_assessment(a, 1, b, 1)
    resolve(direct_vm, contract, assessment_id)
    vexora_id = contract.propose_vexora(assessment_id, 0, 0)
    with direct_vm.prank(direct_bob):
        contract.ratify_vexora(vexora_id)
    agreement_hash = contract.get_vexora(vexora_id)["agreement_hash"]
    contract.pause_policy(a, True)
    assert contract.is_vexora_active(vexora_id, agreement_hash)
...
def test_successor_race_second_child_fails(direct_vm, direct_deploy, direct_alice, direct_bob):
    ...
    assessment_id = contract.open_assessment(a, 1, b, 1)
    resolve(direct_vm, contract, assessment_id)
    parent = contract.propose_vexora(assessment_id, 0, 0)
    with direct_vm.prank(direct_bob):
        contract.ratify_vexora(parent)
    child_a = contract.propose_vexora(assessment_id, 0, parent)
    child_b = contract.propose_vexora(assessment_id, 0, parent)
    with direct_vm.prank(direct_bob):
        contract.ratify_vexora(child_a)
    with direct_vm.expect_revert("parent vexora is no longer active"):
        with direct_vm.prank(direct_bob):
            contract.ratify_vexora(child_b)
    assert contract.get_vexora(parent)["status_name"] == "SUPERSEDED"
...
def test_parent_must_be_active_for_successor(direct_vm, direct_deploy, direct_alice, direct_bob):
    ...
    assessment_id = contract.open_assessment(a, 1, b, 1)
    resolve(direct_vm, contract, assessment_id)
    with direct_vm.expect_revert("unknown vexora"):
        contract.propose_vexora(assessment_id, 0, 999)
