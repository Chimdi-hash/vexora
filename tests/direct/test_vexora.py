"""Direct-mode protocol tests for Vexora."""

import json
import re

CONTRACT = "contracts/vexora.py"
...
def test_bilateral_activation_and_hash_pinning(direct_vm, direct_deploy, direct_alice, direct_bob):
    ...
    assert contract.get_vexora(vexora_id)["status_name"] == "PROPOSED"
    with direct_vm.prank(direct_bob):
        contract.ratify_vexora(vexora_id)
    vexora = contract.get_vexora(vexora_id)
    assert vexora["status_name"] == "ACTIVE"
    assert contract.is_vexora_active(vexora_id, vexora["agreement_hash"])
    assert not contract.is_vexora_active(vexora_id, "00" * 32)
...
def test_terms_are_original_source_clauses(direct_vm, direct_deploy, direct_alice, direct_bob):
    ...
    resolve(direct_vm, contract, assessment_id)
    vexora_id = contract.propose_vexora(assessment_id, 0, 0)
    terms = contract.get_vexora_terms(vexora_id)
