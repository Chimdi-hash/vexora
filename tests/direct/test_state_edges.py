"""Boundary and state-machine coverage for the versioned Vexora protocol."""

import json

from test_vexora import B, DOMAIN, SAFE_COMPATIBLE, A, create_pair, resolve, setup_domain


def test_empty_domain_reverts(direct_vm, direct_deploy, direct_alice):
    direct_vm.sender = direct_alice
    contract = direct_deploy("contracts/vexora.py")
...
def test_rejection_is_terminal(direct_vm, direct_deploy, direct_alice, direct_bob):
    ...
    contract.reject_vexora(vexora_id)
    assert contract.get_vexora(vexora_id)["status_name"] == "REJECTED"
...
def test_expiry_past_and_now_revert(direct_vm, direct_deploy, direct_alice, direct_bob):
    ...
    with direct_vm.expect_revert("expiry must be in the future"):
        contract.propose_vexora(assessment_id, now, 0)
...
def test_expiry_maximum_allowed_and_plus_one_rejected(direct_vm, direct_deploy, direct_alice, direct_bob):
    ...
    vexora_id = contract.propose_vexora(assessment_id, max_expiry, 0)
    assert contract.get_vexora(vexora_id)["expires_at"] == max_expiry
    with direct_vm.expect_revert("lifetime exceeds"):
        contract.propose_vexora(assessment_id, max_expiry + 1, 0)
...
def test_consumer_interface_requires_exact_agreement_hash(direct_vm, direct_deploy, direct_alice, direct_bob):
    ...
    resolve(direct_vm, contract, assessment_id)
    vexora_id = contract.propose_vexora(assessment_id, 0, 0)
    assert not contract.is_vexora_active(vexora_id, "wrong")
    assert contract.get_vexora_terms(vexora_id)
