...
Semantic compatibility is not consent. Only the two policy owners can ratify, and a proposal remains `PROPOSED` after one side. Pausing a policy blocks new proposals and pending ratification but does not retroactively revoke an already active vexora. This preserves completed bilateral consent; revocation is deliberately not part of the current primitive.
...
- Vexora never moves value, holds funds, or calls arbitrary downstream contracts.
